...
--Step 1: Create the Database1
CREATE DATABASE MotorsCertification;

--Use DataBase
USE MotorsCertification;

--Step 1.1: Create Tables orderdetails, employees, payments, products, customers, offices, and orders tables.
--Step 2: Insert Data
--Insert data into the orderdetails, employees, payments, products, customers, offices, and orders tables.

--1.1: Create offices Table
...
--2.1: insert records into the offices table.
...
--Step:5. Use a select statement to verify all insertions as well as updates.
select * from offices;
...
--2.2 Insert records into employees table
...
--Step 5:select statement to verify all insertions as well as updates in employees table.
...
--2.3: Insert records in customers table
...
VALUES
	...
	(148,'Dragon Souveniers, Ltd.','Natividad','Eric','+65 221 7555','Bronz Sok.','Bronz Apt. 3/6 Tesvikiye','Singapore',NULL,'079903','Singapore',1076,'103800.00'),
...
--Step 5:select statement to verify all insertions as well as updates in customers table.
select * from customers;


--1.4: Create orders Table
CREATE TABLE orders (
...
--2.4: insert records in orders tbale
...
(10107,'2003-02-24','2003-03-03','2003-02-26','Shipped','Difficult to negotiate with customer. We need more marketing materials',128),
...
(10118,'2003-04-21','2003-04-29','2003-04-26','Shipped','Customer has worked with some of our vendors in the past and is aware of their MSRP',166),

(10119,'2003-04-28','2003-05-05','2003-05-02','Shipped',NULL,167);
--Step 5:select statement to verify all insertions as well as updates in orders table.
select * from orders;


--1.5: Create productlines Table
...
--2.5: insert records in productlines table
...
--Step 5:select statement to verify all insertions as well as updates in productlines table.
Select * from productlines;


--1.6: Create products Table
...
--2.6: Insert records into products table
...
('S10_4698','2003 Harley-Davidson Eagle Drag Bike','Ships','1:10','Red Start Diecast','Model features, official Harley Davidson logos and insignias, detachable rear wheelie bar, heavy diecast metal with resin parts, authentic multi-color tampo-printed graphics, separate engine drive belts, free-turning front fork, rotating tires and rear racing slick, certificate of authenticity, detailed engine, display stand\r\n, precision diecast replica, baked enamel finish, 1:10 scale model, removable fender, seat and tank cover piece for displaying the superior detail of the v-twin engine',5582,'91.02','193.66'),

('S10_4757','1972 Alfa Romeo GTA','Trains','1:10','Motor City Art Classics','Features include: Turnable front wheels; steering function; detailed interior; detailed engine; opening hood; opening trunk; opening doors; and detailed chassis.',3252,'85.68','136.00'),
...
--Step 5:select statement to verify all insertions as well as updates in products table.
Select * from products;


--1.7: Create orderdetails Table
...
--2.7: insert recodes into orderdetails table
insert into orderdetails 
values
    (10100,'S10_1678',30,'136.00',3),

	(10101,'S10_1949',50,'55.09',2),

	(10102,'S10_2016',22,'75.46',4),

	(10103,'S10_4698',49,'35.29',1),

	(10104,'S10_4757',25,'108.06',4),

	(10105,'S10_4962',26,'167.06',1),

	(10106,'S12_1099',45,'32.53',3),

	(10107,'S12_1108',46,'44.35',2),

	(10108,'S12_1666',39,'95.55',2),

	(10109,'S12_2823',41,'43.13',1),

	(10110,'S12_3148',26,'214.30',11),

	(10111,'S12_3380',42,'119.67',4),

	(10112,'S12_3891',27,'121.64',8),

	(10113,'S12_3990',35,'94.50',10),

	(10114,'S12_4473',22,'58.34',2),

	(10115,'S12_4675',27,'92.19',12),

	(10116,'S18_1097',35,'61.84',14),

	(10117,'S18_1129',25,'86.92',13),

	(10118,'S18_1342',46,'86.31',16),

	(10119,'S18_1367',36,'98.07',5);
--Step 5:select statement to verify all insertions as well as updates in orderdetails table.
select * from orderdetails;

--1.8: Create payments Table
...
--2.8: insert records into payments table.
...
--Step 5:select statement to verify all insertions as well as updates in payments table.
select * from payments;



-- 4: Delete Useless Columns  in productlines.
ALTER TABLE productlines DROP COLUMN htmlDescription, image;
-- select statement to verify  updates in productlines table. 
select * from productlines;

--5. Select Statement to Verify Data
SELECT * FROM employees;

--6. Find Highest and Lowest Payment Amount
SELECT MAX(amount) AS HighestAmount, MIN(amount) AS LowestAmount FROM payments;
SELECT * FROM payments;

--7. unique count of customerName from customers.SELECT COUNT(DISTINCT customerName) FROM customers;--This query will give the total number of unique customer names in the customers table.
SELECT * FROM customers;

--8. Create and Drop Views
--8.1. Create a view from customers and payments named cust_payment and select customerName, amount, contactLastName, contactFirstName who have paid.Truncate and Drop the view after operation.

-- Any other SQL code you might have before
GO -- This separates the previous statements from the next batch
...
GO
-- Select from the view cust_payment selects data from customers and payments where the customerNumber matches in both tables.
SELECT * FROM cust_payment;


--The view cust_payment selects data from customers and payments where the customerNumber matches in both tables.
--8.2. Truncate and Drop View

--In SQL Server, TRUNCATE is only used for tables to quickly remove all rows, but views do not store data themselves — they just display data from underlying tables. Therefore, you cannot truncate or delete rows directly from a view.
--TRUNCATE TABLE cust_payment;

-- When finished, drop the view
DROP VIEW cust_payment;--To remove a view, you would use the DROP VIEW command instead of TRUNCATE.

-- 9. Create a stored procedure on products which displays productLine for Classic Cars.
--Stored Procedure for Classic Cars
-- Separate previous statements with a GO to indicate a new batch
GO

CREATE PROCEDURE GetClassicCarsProductLine
AS
BEGIN
    -- Select the productLine for Classic Cars
    SELECT productLine
    FROM products
...
-- Optionally, execute the procedure
EXEC GetClassicCarsProductLine;
GO


--10. Function to Get the creditLimit of customers less than 96800.
GO

CREATE FUNCTION GetLowCreditCustomers()  --CREATE FUNCTION: This is the SQL command to create a user-defined function.
RETURNS TABLE                          --RETURNS TABLE: This specifies that the function will return a table.
AS
RETURN									--RETURN: Inside the RETURN statement, we use a SELECT query to fetch customers whose creditLimit is less than 96,800.
(
  SELECT customerName, creditLimit 
  FROM customers 
  WHERE creditLimit < 96800
);
-- Call the function to retrieve results
SELECT * FROM GetLowCreditCustomers();
GO
...
--11.Create Trigger to store transaction record for employee table which displays employeeNumber, lastName, FirstName and office code upon insertion
 -- Trigger for Employee Insertion

CREATE TRIGGER after_employee_insert
ON employees
AFTER INSERT     --AFTER INSERT: This specifies that the trigger should be fired after a new row is inserted into the employees table.
AS
BEGIN
    -- Select the newly inserted employee details
    SELECT 
        inserted.employeeNumber, --inserted table: This is a special virtual table available within the trigger that contains the newly inserted row.
        inserted.lastName,       --The SELECT statement inside the trigger outputs the employee details (employeeNumber, lastName, firstName, and officeCode).
        inserted.firstName, 
        inserted.officeCode
    FROM 
        inserted;

    -- Optionally, you can log or store the inserted records in another table
  
END;
GO

--12.Create a Trigger to display customer number if the amount is greater than 10,000-- Trigger for Payment Amount Greater than 10,000
GO
...
BEGIN
    ...
    SELECT @CustomerNumber = customerNumber, @PaymentAmount = amount
    FROM inserted;

    -- Check if the payment amount is greater than 10,000
    IF @PaymentAmount > 10000
    BEGIN
        -- Raise an error with the customer number
        RAISERROR('Customer Number: %d has a payment amount greater than 10,000.', 16, 1, @customerNumber);
    END
END;
 -- This will trigger the RAISERROR since the payment is greater than 10,000.

'''--AFTER INSERT: The trigger runs after a new record is inserted into the Payments table.
--inserted: This special table contains the new rows. The trigger retrieves the CustomerNumber and PaymentAmount from this table.
--RAISERROR: If the PaymentAmount exceeds 10,000, it raises an error displaying the customer number.
'''
...
--13.2. HR Role
CREATE ROLE HR;
-- HRRole: Full access to the Employee and Offices tables only
GRANT take ownership ON dbo.employees TO HR;
GRANT take ownership ON dbo.offices TO HR;
...
-- Create Logins
CREATE LOGIN AdminUser WITH PASSWORD = 'AdminUserPassword1!';
CREATE LOGIN HRUser WITH PASSWORD = 'HRUserPassword2!';
CREATE LOGIN EmployeeUser WITH PASSWORD = 'EmployeeUserPassword3!';
